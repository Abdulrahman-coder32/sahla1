...
@Component({
  ...
  styles: [`
    ...
    .my-message { flex-direction: row; }        /* رسائلي على اليسار */
    .other-message { flex-direction: row-reverse; } /* رسائل الطرف التاني على اليمين */
...
export class InboxComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // باقي الكود زي ما هو بالضبط (ngOnInit, sendMessage, scrollToBottom, إلخ)
  // مش هغير أي منطق، بس حطيت الكود كامل عشان تستبدل الملف كله مرة واحدة
  ngOnInit() {
  ...
  onImageError(event: Event) {
    ...
    if (img.src !== this.defaultImage) img.src = this.defaultImage;
  }
  ...
  sendMessage() {
    ...
    this.api.sendMessage({ application_id: this.selectedApp._id, message: text }).subscribe({
      next: (savedMsg: any) => {
        ...
        if (index !== -1) this.messages[index] = this.normalizeMessage(savedMsg);
      },
      error: () => {
