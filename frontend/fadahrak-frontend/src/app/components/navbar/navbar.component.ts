...
@Component({
  ...
  template: `
    <nav class="bg-white shadow-lg sticky top-0 z-50 border-b border-gray-200">
          ...
          <div class="hidden md:flex items-center gap-6 lg:gap-8 xl:gap-10">
            ...
            <ng-container *ngIf="user; else guestDesktop">
              ...
              <div class="relative group">
                ...
                <div class="absolute end-0 mt-3 w-80 lg:w-96 bg-white rounded-xl shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
              ...
              <!-- Profile Dropdown (مع صورة الملف الشخصي والاسم) -->
              <div class="relative group">
                ...
                <div class="absolute end-0 mt-3 w-56 lg:w-64 bg-white rounded-xl shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
          ...
          <div class="md:hidden flex items-center gap-3 lg:gap-4">
            <ng-container *ngIf="user">
              <button (click)="mobileNotificationsOpen = !mobileNotificationsOpen; mobileMenuOpen = false" class="relative p-2">
                <i class="fas fa-bell text-xl lg:text-2xl text-gray-700"></i>
                <ng-container *ngIf="notificationCount$ | async as count">
            ...
            <button (click)="mobileMenuOpen = !mobileMenuOpen; mobileNotificationsOpen = false" class="p-2">
              <div class="w-7 h-7 lg:w-8 lg:h-8 flex flex-col justify-center gap-1">
                <span [ngClass]="{'rotate-45 translate-y-2 lg:translate-y-2.5': mobileMenuOpen}" class="block h-0.5 w-full bg-gray-800 rounded transition-all duration-300"></span>
      ...
      <div [ngClass]="{
      ...
      }" class="fixed top-14 sm:top-16 left-0 right-0 bg-white shadow-xl z-50 transition-transform duration-500 ease-in-out md:hidden border-t border-gray-200 rounded-b-xl">
        ...
        <div *ngIf="mobileNotificationsOpen" class="p-4">
          ...
          <a routerLink="/notifications" (click)="closeMobileMenu()" class="block mt-4 p-4 text-center bg-gray-50 hover:bg-gray-100 text-gray-600 font-medium rounded-xl">
            عرض جميع الإشعارات
          </a>
        </div>
        ...
        <div *ngIf="mobileMenuOpen && !mobileNotificationsOpen" class="p-4">
          <ng-container *ngIf="user; else guestMobile">
            <a routerLink="/inbox" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">الرسائل</a>
            <a [routerLink]="user.role === 'shop_owner' ? '/owner-dashboard' : '/seeker-dashboard'"
               (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">لوحة التحكم</a>
            <a routerLink="/profile" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">الملف الشخصي</a>
            <hr class="my-4">
          ...
          <ng-template #guestMobile>
            <a routerLink="/login" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">دخول</a>
            <a routerLink="/signup" (click)="closeMobileMenu()" class="block py-3 px-4 text-base bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl text-right font-medium">إنشاء حساب</a>
          </ng-template>

          <hr class="my-4">
          <div class="space-y-2">
            <a routerLink="/" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">الرئيسية</a>
            <a routerLink="/jobs" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">الوظائف</a>
            <a routerLink="/about" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">عننا</a>
            <a routerLink="/contact" (click)="closeMobileMenu()" class="block py-3 px-4 text-base hover:bg-gray-100 rounded-xl text-right">اتصل بنا</a>
          </div>
        </div>
      </div>
    </nav>
  ...
  styles: [`
    ...
    .active-link::after {
      content: '';
      @apply absolute bottom-[-6px] left-0 right-0 h-0.5 bg-indigo-600 rounded-full;
    }
    .btn-primary {
