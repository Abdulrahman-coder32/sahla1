...
@Component({
  ...
  styles: [`
    ...
    .nav-link {@apply relative text-gray-700 text-base font-medium px-4 py-2 transition-all duration-300 hover:text-indigo-600; width:fit-content;}
    .nav-link::after{content:'';@apply absolute bottom-0 w-0 h-0.5 bg-indigo-600 rounded-full transition-all duration-500;left:10px;}
    .nav-link:hover::after,.active-link::after{@apply w-full;}
    .active-link{@apply text-indigo-600 font-bold;}
  `]
})
